...
public:
  PcapTcpPacketData(PcapIPv4Packet packet);
  inline PcapTcpPacketData(const PcapTcpPacketData &other)
  ...
  inline PcapIPv4Packet ip() const { return _ip; }
  QString english() const;
};

class PcapTcpPacket {
private:
  QSharedDataPointer<PcapTcpPacketData> d;

public:
  inline explicit PcapTcpPacket(PcapIPv4Packet packet = PcapIPv4Packet()) {
    d = new PcapTcpPacketData(packet);
  }
  inline PcapTcpPacket(const PcapTcpPacket &other) : d(other.d) { }
